// frontend/src/pages/Process6Page.jsx
...
const Process6Page = () => {
  ...
  const fetchDocumentationFiles = useCallback(async () => {
    try {
      const response = await API.getProcess6Documentation(id);
  ...
  const fetchA3DocumentationFiles = useCallback(async () => {
    try {
      const response = await API.getProcess6A3Documentation(id);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const process6Rate = processRatesResponse.find(
          (rate) => rate.id_process === 6
        );
        if (process6Rate) {
          setProcessInfo(process6Rate);
        }

        // Fetch work steps for Process 6
        const workStepsResponse = await API.getWorkSteps(6);
  ...
  // Get deadline information for Process 6
  const getDeadlineInfo = () => {
    const process6 = processes.find((p) => p.id_process === 6);
    ...
    return {
      text: `${process6.deadline} ngày trước thời gian dự kiến`,
      date: calculateDeadlineDate(plan?.plan_date, process6.deadline),
    };
  };
  ...
  const uploadDocumentationFiles = async () => {
    ...
    try {
      await API.uploadProcess6DocumentationFile(id, documentationFiles);
  ...
  const uploadA3DocumentationFiles = async () => {
    ...
    setUploadingA3(true);
    try {
      await API.uploadProcess6A3DocumentationFile(id, a3DocumentationFiles);
  ...
  const deleteDocumentationFile = async (index) => {
    ...
    try {
      const response = await API.deleteProcess6Documentation(id, index);
      if (response.success) {
  ...
  const deleteA3DocumentationFile = async (index) => {
    ...
    try {
      const response = await API.deleteProcess6A3Documentation(id, index);
      if (response.success) {
  ...
  return (
    <Container component="main" maxWidth="xl">
      ...
      >
        ...
        >
          <Typography variant="h5" fontWeight="bold">
            CÁC BƯỚC CÔNG VIỆC HỌP CHUYỂN ĐỔI MÃ HÀNG MỚI
          </Typography>
        </Box>
