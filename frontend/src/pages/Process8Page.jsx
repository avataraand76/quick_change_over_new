// frontend/src/pages/Process8Page.jsx
...
const Process8Page = () => {
  ...
  const fetchDocumentationFiles = useCallback(async () => {
    try {
      const response = await API.getProcess8Documentation(id);
  ...
  const fetchA3DocumentationFiles = useCallback(async () => {
    try {
      const response = await API.getProcess8A3Documentation(id);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const process8Rate = processRatesResponse.find(
          (rate) => rate.id_process === 8
        );
        if (process8Rate) {
          setProcessInfo(process8Rate);
        }

        // Fetch work steps for Process 8
        const workStepsResponse = await API.getWorkSteps(8);
        ...
        // Check if Process 8 is overdue
        const process8 = processesResponse.find((p) => p.id_process === 8);
        if (process8) {
  ...
  // Get deadline information for Process 8
  const getDeadlineInfo = () => {
    const process8 = processes.find((p) => p.id_process === 8);
    if (!process8) return { text: "Đang tải...", date: null };

    if (!process8.deadline || process8.deadline === 0) {
      return {
        text: `Cùng ngày với ${
    ...
    return {
      text: `${process8.deadline} ngày trước ${
        plan?.actual_date ? "thời gian thực tế" : "thời gian dự kiến"
      }`,
  ...
  const uploadDocumentationFiles = async () => {
    ...
    try {
      await API.uploadProcess8DocumentationFile(id, documentationFiles);
  ...
  const uploadA3DocumentationFiles = async () => {
    ...
    setUploadingA3(true);
    try {
      await API.uploadProcess8A3DocumentationFile(id, a3DocumentationFiles);
  ...
  const deleteDocumentationFile = async (index) => {
    ...
    try {
      const response = await API.deleteProcess8Documentation(id, index);
      if (response.success) {
  ...
  const deleteA3DocumentationFile = async (index) => {
    ...
    try {
      const response = await API.deleteProcess8A3Documentation(id, index);
      if (response.success) {
  ...
  return (
    <Container component="main" maxWidth="xl">
      ...
      >
        ...
        >
          <Typography variant="h5" fontWeight="bold">
            CÁC BƯỚC CÔNG VIỆC ĐÀO TẠO BCV MỚI CHO CÔNG NHÂN
          </Typography>
        </Box>
