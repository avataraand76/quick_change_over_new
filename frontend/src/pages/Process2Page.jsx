// frontend/src/pages/Process2Page.jsx
...
const Process2Page = () => {
  ...
  const fetchDocumentationFiles = useCallback(async () => {
    try {
      const response = await API.getProcess2Documentation(id);
  ...
  const fetchA3DocumentationFiles = useCallback(async () => {
    try {
      const response = await API.getProcess2A3Documentation(id);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const process2Rate = processRatesResponse.find(
          (rate) => rate.id_process === 2
        );
        if (process2Rate) {
          setProcessInfo(process2Rate);
        }

        // Fetch work steps for Process 2
        const workStepsResponse = await API.getWorkSteps(2);
        ...
        // Check if Process 2 is overdue
        const process2 = processesResponse.find((p) => p.id_process === 2);
        if (process2 && planResponse.actual_date) {
  ...
  // Get deadline information for Process 2
  const getDeadlineInfo = () => {
    const process2 = processes.find((p) => p.id_process === 2);
    if (!process2) return { text: "Đang tải...", date: null };

    if (!process2.deadline || process2.deadline === 0) {
      return { text: "Cùng ngày với thời gian thực tế", date: null };
    }

    return {
      text: `${process2.deadline} ngày trước thời gian thực tế`,
      date: calculateDeadlineDate(plan?.actual_date, process2.deadline),
    };
  };
  ...
  const uploadDocumentationFiles = async () => {
    ...
    try {
      await API.uploadProcess2DocumentationFile(id, documentationFiles);
  ...
  const uploadA3DocumentationFiles = async () => {
    ...
    setUploadingA3(true);
    try {
      await API.uploadProcess2A3DocumentationFile(id, a3DocumentationFiles);
  ...
  return (
    <Container component="main" maxWidth="xl">
      ...
      >
        ...
        >
          <Typography variant="h5" fontWeight="bold">
            CÁC BƯỚC CÔNG VIỆC HỌP CÔNG TÁC CHUẨN BỊ SẢN XUẤT MÃ HÀNG MỚI
          </Typography>
        </Box>
      ...
      >
        ...
        >
          <Typography variant="h5" fontWeight="bold">
            HỌP CÔNG TÁC CHUẨN BỊ SẢN XUẤT MÃ HÀNG MỚI
          </Typography>
        </Box>
