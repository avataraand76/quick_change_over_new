// frontend/src/pages/Process7Page.jsx
...
const Process7Page = () => {
  ...
  const fetchDocumentationFiles = useCallback(async () => {
    try {
      const response = await API.getProcess7Documentation(id);
  ...
  const fetchA3DocumentationFiles = useCallback(async () => {
    try {
      const response = await API.getProcess7A3Documentation(id);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const process7Rate = processRatesResponse.find(
          (rate) => rate.id_process === 7
        );
        if (process7Rate) {
          setProcessInfo(process7Rate);
        }

        // Fetch work steps for Process 7
        const workStepsResponse = await API.getWorkSteps(7);
        ...
        // Check if Process 7 is overdue
        const process7 = processesResponse.find((p) => p.id_process === 7);
        if (process7) {
  ...
  // Get deadline information for Process 7
  const getDeadlineInfo = () => {
    const process7 = processes.find((p) => p.id_process === 7);
    ...
    return {
      text: `${process7.deadline} ngày trước ${
        plan?.actual_date ? "thời gian thực tế" : "thời gian dự kiến"
      }`,
  ...
  const uploadDocumentationFiles = async () => {
    ...
    try {
      await API.uploadProcess7DocumentationFile(id, documentationFiles);
  ...
  const uploadA3DocumentationFiles = async () => {
    ...
    setUploadingA3(true);
    try {
      await API.uploadProcess7A3DocumentationFile(id, a3DocumentationFiles);
  ...
  const deleteDocumentationFile = async (index) => {
    ...
    try {
      const response = await API.deleteProcess7Documentation(id, index);
      if (response.success) {
  ...
  const deleteA3DocumentationFile = async (index) => {
    ...
    try {
      const response = await API.deleteProcess7A3Documentation(id, index);
      if (response.success) {
  ...
  return (
    <Container component="main" maxWidth="xl">
      ...
      >
        ...
        >
          <Typography variant="h5" fontWeight="bold">
            CÁC BƯỚC CÔNG VIỆC CUNG CẤP BTP, PL
          </Typography>
        </Box>
