...
const Process4Page = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Check if Process 4 is overdue
        const process4 = processesResponse.find((p) => p.id_process === 4);
        if (process4) {
  ...
  const getDeadlineInfo = () => {
    ...
    return {
      text: `${process4.deadline} ngày trước ${
        plan?.actual_date ? "thời gian thực tế" : "thời gian dự kiến"
      }`,
  ...
  const handleConfirmDelete = async () => {
    try {
      const response = await API.deleteProcess4Documentation(
        id,
        deletingFileIndex
      );
      if (response.success) {
  ...
  const handleConfirmA3Delete = async () => {
    try {
      const response = await API.deleteProcess4A3Documentation(
        id,
        deletingA3FileIndex
      );
      if (response.success) {
