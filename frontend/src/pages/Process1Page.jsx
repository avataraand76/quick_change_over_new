...
const Process1Page = () => {
  ...
  const fetchDocumentationFiles = useCallback(async () => {
    try {
      const response = await API.getProcess1Documentation(id);
  ...
  const fetchA3DocumentationFiles = useCallback(async () => {
    try {
      const response = await API.getProcess1A3Documentation(id);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const process1Rate = processRatesResponse.find(
          (rate) => rate.id_process === 1
        );
        if (process1Rate) {
          setProcessInfo(process1Rate);
        }

        // Fetch work steps for Process 1
        const workStepsResponse = await API.getWorkSteps(1);
  ...
  // Get deadline information for Process 1
  const getDeadlineInfo = () => {
    const process1 = processes.find((p) => p.id_process === 1);
    if (!process1) return { text: "Đang tải...", date: null };

    if (!process1.deadline || process1.deadline === 0) {
      return { text: "Cùng ngày với thời gian thực tế", date: null };
    }

    return {
      text: `${process1.deadline} ngày trước thời gian thực tế`,
      date: calculateDeadlineDate(plan?.actual_date, process1.deadline),
    };
  };
  ...
  const deleteDocumentationFile = async (index) => {
    ...
    try {
      const response = await API.deleteProcess1Documentation(id, index);
      if (response.success) {
  ...
  const deleteA3DocumentationFile = async (index) => {
    ...
    try {
      const response = await API.deleteProcess1A3Documentation(id, index);
      if (response.success) {
  ...
  return (
    <Container component="main" maxWidth="xl">
      ...
      >
        ...
        >
          <Typography variant="h5" fontWeight="bold">
            CÁC BƯỚC CÔNG VIỆC NGHIÊN CỨU MẪU GỐC
          </Typography>
        </Box>
